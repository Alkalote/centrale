package fr.hb.jg.centrale.configuration;
import fr.hb.jg.centrale.embeddable.UserListingID;
import fr.hb.jg.centrale.entity.*;
import fr.hb.jg.centrale.repository.*;
            ...
            for (long i = 1L; i <= 500L; i++) {
                ...
                user.setBirthAt(generateRandomDate(
                        Instant.now().minusSeconds(999999999)
                                ...
                                .minusSeconds(999999999)).toLocalDate().atStartOfDay());
        ...
        for (User user : users) {
            for (int i = 0; i < random.nextInt(2, 5); i++) {
                ...
                favorite.setCreatedAt(generateRandomDate(
                        Instant.now().minusSeconds(999999999)));
                favorite.setId(new UserListingID(listing.getUuid(), user.getUuid()));;
